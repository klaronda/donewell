...
export default async function handler(req: any, res: any) {
  ...
  const siteId = process.env.SITE_ID || 'dw_prod_001'
  const siteName = process.env.SITE_NAME || 'donewellco.com'
  ...
  // CMS check - fetch 1 project with 2000ms timeout
  if (supabaseUrl && supabaseAnonKey) {
    ...
    // Forms check - verify leads table is accessible
    try {
      const controller = new AbortController()
      const timeoutId = setTimeout(() => controller.abort(), 2000)
