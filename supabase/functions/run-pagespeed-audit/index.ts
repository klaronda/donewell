...
serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    let { url, lead_id }: AuditRequest = await req.json()
    ...
    // Check rate limits using database function (if it exists)
    try {
      const { data: rateLimitCheck, error: rateLimitError } = await supabase.rpc('check_rate_limits', {
        url_to_check: url
      })

      if (!rateLimitError && rateLimitCheck && Array.isArray(rateLimitCheck) && rateLimitCheck.length > 0) {
        const rateLimitResult = rateLimitCheck[0]
        if (rateLimitResult && typeof rateLimitResult.can_proceed === 'boolean' && !rateLimitResult.can_proceed) {
          return new Response(
            JSON.stringify({ 
      ...
      // Rate limit function may not exist, continue without it
    ...
    // Mark any existing audits as not latest
    await supabase
      .from('site_audits')
